...
class PairedImgDataset(Dataset):
    def __init__(self, data_source, mode, crop=256, random_resize=None):
        if not mode in ['train', 'eval', 'test']:
            raise Exception('The mode should be "train", "eval" or "test".')
        ...
        self.img_paths = sorted(glob.glob(data_source + '/' + mode + '/noisy' + '/*.*'))
...
class SingleImgDataset(Dataset):
    def __init__(self, data_source):
        ...
        self.img_paths = sorted(glob.glob(data_source + '/*.*'))
